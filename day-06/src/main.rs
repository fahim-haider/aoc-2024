...
fn count_guard_positions (map: &mut [[char; MAP_SIZE]; MAP_SIZE], 
                    guard: &mut Guard) -> (bool,HashSet<(i32,i32)>) {
...
fn main() {
    let (map, guard) = create_map("src/input.in");
    let start_time = Instant::now();
    let (_is_loop,mut step_pos) = count_guard_positions(&mut map.clone(), &mut guard.clone());
    let duration = start_time.elapsed();
    println!("Part 1: {:?}, {:?}", _is_loop, step_pos.len());
    println!("Time taken for Part 1 = {:?}", duration);
    let start_time = Instant::now();
    let count = time_to_brute_force(&mut map.clone(), &mut guard.clone(),&mut step_pos);
    let duration = start_time.elapsed();
